...
<input type="hidden" value="${subType}" id = "subType"> 
		...
		<table>
			<tr>
				<c:if test="${prodTag eq 'prodTag'}">
					<td width="150">
					<a href="styleProdTag.user?cate=${cate}&subType=all"><font  size="5px" class="midleTopFont">전체</font></a></td>
					<c:forEach var="dto" items="${subList}">
						<td width="150">
							<a href="styleProdTag.user?cate=${dto.cate_type}&subType=${dto.cate_subType}"><font size="5px" class="midleTopFont">${dto.cate_kr_subType}</font></a>
						</td>
					</c:forEach>
				...
				<c:if test="${prodTag ne 'prodTag'}">
					<c:forEach var="name" items="${showHashTagList}">
						<td width="150"><a href="javascript:sendHashTag('${name}')">#${name}</a>
						</td>
					</c:forEach>
				</c:if>
			</tr>
			...
			<tr>
				<c:forEach var="dto" items="${list}">
					...
					<c:if test="${dto.checkBan!=1}">
						<td><br> <a href="styleView.user?styleNum=${dto.style_num}&userNum=${dto.user_num}"><img
